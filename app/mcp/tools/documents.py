...
def register_document_tools(mcp: FastMCP) -> None:
    ...
    async def list_documents(limit: int = 20) -> list[dict[str, Any]]:
        ...
        How LLM uses this:
        1. The model can understand what documents exist in the system.
        2. Then it can call get_document() or read_document_text() by document ID.
        """
    ...
    async def get_document(document_id: int) -> dict[str, Any]:
        """
        Returns one document metadata by ID.
        """
        if document_id < 1:
    ...
    async def read_document_text(
        ...
        """
        Reads a real text file from disk using storage_path from the database.

        Works well for txt, md, json, csv.
        Does not properly parse PDF/DOCX yet.
        """
        if document_id < 1:
            return {"error": "Document id must be positive"}
    ...
    async def search_documents(
        ...
        """
        Searches documents by metadata: filename and content_type.

        This is not RAG.
        This tool does not search inside document content.
        """
        query = query.strip()
            ...
            async with async_session() as session:
                ...
                stmt = (
                    ...
                    .where(
                        ...
                        | Document.content_type.ilike(search_pattern)
                    )
                    .order_by(Document.created_at.desc())
                    .limit(limit)
                )
